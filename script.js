...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    const masterDB = {
        ...
        "Klasik Araçlar": { "Klasik Otomobil": ["Chevrolet Impala", "Ford Mustang", "Mercedes-Benz W114", "VW Beetle (Tosbağa)"], "Klasik Arazi Aracı": ["Jeep CJ", "Land Rover Series"] },
    ...
    // İLAN FORMU EKSPERTİZ GİZLEME KONTROLÜ
            ...
            listCont.innerHTML += `
                <a href="detay.html?id=${c.id}" class="card">
                    ...
                    <div class="card-content">
                        <h3>${c.title}</h3><p class="price">${c.price}</p><p class="details">${c.city} • ${c.category}</p>
                    </div>
                </a>`;
        });
    };
    ...
    // İLAN EKLEME (TAM KAPSAMLI - EKSPERTİZ, TAKAS, HASAR, İL/İLÇE DAHİL)
    ...
    // DETAY SAYFASI (EKSPERTİZ TABLOSU VE BİLGİLERİ GÖSTER)
        ...
        if(id) {
            db.collection('listings').doc(id).get().then(d => {
                if(d.exists) {
                    ...
                    document.getElementById('det-loc').textContent = c.city + ' / ' + c.district; document.getElementById('det-brand').textContent = c.brand; document.getElementById('det-model').textContent = c.model; document.getElementById('det-year').textContent = c.year; document.getElementById('det-km').textContent = c.km + ' km'; document.getElementById('det-gear').textContent = c.gear; document.getElementById('det-fuel').textContent = c.fuel; document.getElementById('det-phone').textContent = c.phone; document.getElementById('det-desc').textContent = c.description; document.getElementById('det-cat').textContent = c.category;
                    ...
                    if(c.type === 'Satılık') {
                        ...
                        if(c.hasarKaydi === 'Var') { hasarSpan.textContent = 'Var (Ağır Hasarlı)'; hasarSpan.style.color = '#d32f2f'; } else { hasarSpan.textContent = 'Yok'; hasarSpan.style.color = '#388e3c'; }
                        document.getElementById('det-tramer').textContent = c.tramer ? c.tramer + " TL" : "Belirtilmemiş";
                    }

                    // EKSPERTİZ TABLOSU
                    const expSec = document.getElementById('expertise-section'); const expGrid = document.getElementById('det-expertise');
                    if(expSec && expGrid && c.expertise) {
                        let hasData = false;
                        for (const [parca, durum] of Object.entries(c.expertise)) {
                            if(durum !== 'Belirtilmemiş') {
                                hasData = true; let colorClass = "exp-yok";
                                if(durum.includes('Orijinal')) colorClass = "exp-orijinal"; else if(durum === 'Boyalı') colorClass = "exp-boyali"; else if(durum.includes('Lokal')) colorClass = "exp-lokal"; else if(durum.includes('Değişen') || durum.includes('İşlemli')) colorClass = "exp-degisen";
