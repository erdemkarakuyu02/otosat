...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    const masterDB = {
        "Otomobil": {
            ...
            "Audi": ["A1", "A3", "A4", "A5", "A6", "A7", "A8", "R8", "TT", "e-tron GT", "Q Serisi"], 
    ...
    // VİTRİN SEKMELERİ (SATILIK - KİRALIK) KONTROLÜ
    document.querySelectorAll('.type-tab').forEach(tab => {
        tab.addEventListener('click', function(e) {
            e.preventDefault();
            document.querySelectorAll('.type-tab').forEach(t => {
                t.style.background = '#fff'; t.style.color = '#454f5b'; t.style.border = '2px solid #dfe3e8'; t.style.boxShadow = 'none';
            });
    ...
    window.applyFilters = function() {
        ...
        filtered.forEach(c => {
            ...
            listCont.innerHTML += `
                <a href="detay.html?id=${c.id}" class="card" style="display:flex; flex-direction:column; position:relative; background:#fff; border-radius:16px; overflow:hidden; box-shadow:0 6px 20px rgba(0,0,0,0.05); text-decoration:none; color:inherit; border: 1px solid #f0f2f5;">
                    ${statusBadge}
                    ${editBtn}
                    ${!isMyListings ? `<button onclick="toggleFav(event, '${c.id}')" style="position:absolute; top:12px; right:12px; background:rgba(255,255,255,0.95); border:none; border-radius:50%; padding:10px; cursor:pointer; z-index:10; font-size:16px; box-shadow:0 4px 10px rgba(0,0,0,0.1);">${heartIcon}</button>` : ''}
                    <div class="card-img-container" style="position:relative; width:100%; height:210px; background:#fafafa;">
                    ...
                    <div class="card-content" style="padding:20px; display:flex; flex-direction:column; flex-grow:1;">
                        <h3 style="font-size:16px; margin-bottom:8px; color:#1a1c20; font-weight:700;">${c.title}</h3>
                        <p class="price" style="font-size:20px; font-weight:800; color:#e53935; margin-bottom:12px;">${c.price}</p>
                        <p class="details" style="font-size:13px; color:#919eab; margin-top:auto; font-weight:500; border-top:1px solid #f0f2f5; padding-top:12px;">${c.city} • ${c.category}</p>
                    </div>
                </a>`;
        });
    };
