...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    function populateSelects() {
        ...
        // Form İçi (İlan Ekle / Düzenle) Marka-Model Doldurma
        if(fc && fb && fm) {
        ...
        // Filtre İçi Kategori-Marka-Model Doldurma
        if(filCat && filBrand) {
    ...
    // KAPSAMLI FİLTRELEME MOTORU (MODEL, İL, İLÇE, YIL DAHİL)
    window.applyFilters = function() {
        ...
        const isFavPage = document.getElementById('favorites-list') !== null; const isMyListings = document.getElementById('my-listings') !== null;
    ...
    // İLAN DÜZENLEME MOTORU (TAM OTOMATİK HAFIZALI)
    ...
    if (editForm) {
        ...
        db.collection('listings').doc(urlId).get().then(d => {
            if(d.exists) {
                ...
                // Kategori Tetiklemesi
                document.getElementById('form-category').value = c.category;
                document.getElementById('form-category').dispatchEvent(new Event('change'));
                
                // Marka ve Model Tetiklemesi
                document.getElementById('form-brand').value = c.brand;
                document.getElementById('form-brand').dispatchEvent(new Event('change'));
                document.getElementById('form-model').value = c.model;
                
                document.getElementById('form-year').value = c.year || ""; document.getElementById('form-km').value = c.km || "";
                ...
                // İl ve İlçe Tetiklemesi
                document.getElementById('form-city').value = c.city;
                document.getElementById('form-city').dispatchEvent(new Event('change'));
                ...
                // Ekspertiz Verilerini Otomatik Doldur
                if(c.expertise) {
                    for (const [parca, durum] of Object.entries(c.expertise)) {
                        const selectIdMap = {
                            "Kaput": "c-kaput", "Tavan": "c-tavan", "Bagaj Kapağı": "c-bagaj", "Sol Ön Çamurluk": "c-sol-on-camurluk", "Sol Ön Kapı": "c-sol-on-kapi", "Sol Arka Kapı": "c-sol-arka-kapi", "Sol Arka Çamurluk": "c-sol-arka-camurluk", "Sağ Ön Çamurluk": "c-sag-on-camurluk", "Sağ Ön Kapı": "c-sag-on-kapi", "Sağ Arka Kapı": "c-sag-arka-kapi", "Sağ Arka Çamurluk": "c-sag-arka-camurluk",
                            "Şase": "m-sase", "Ön Mesnet": "m-mesnet", "Yakıt Deposu": "m-depo", "Ön Grenaj / Kafa": "m-on-grenaj", "Yan/Arka Grenajlar": "m-yan-grenaj"
                        };
                        const targetId = selectIdMap[parca];
                        if(targetId && document.getElementById(targetId)) document.getElementById(targetId).value = durum;
                    }
                }
            }
        });
    ...
    // ADMIN BÖLÜMÜ TABLO KAYMASI ÇÖZÜLDÜ
    if (document.getElementById('admin-listings-body')) {
        auth.onAuthStateChanged(user => {
            ...
            db.collection('listings').where('status', '==', 'approved').get().then(snap => { 
                ...
                if(tbody) { 
                    tbody.innerHTML = ''; 
                    snap.empty ? tbody.innerHTML = '<tr><td colspan="5">Yok</td></tr>' : snap.forEach(d => { 
                        const c = d.data(); 
                        tbody.innerHTML += `<tr>
                            <td><img src="${c.image}" style="width:80px; height:50px; object-fit:cover; border-radius:4px;"></td>
